...
public class NameRecord extends RecordData
{
   ...
   */
  private final List<NameRange> ranges = new ArrayList<>();
...
//        logger.warn("This name has no cell references: " + name);
      ...
      if (data[pos] == cellReference) {
        ...
        while (pos < data.length) {
          int sheet1 = IntegerHelper.getInt(data[pos + 1], data[pos + 2]);
          int r1 = IntegerHelper.getInt(data[pos + 3], data[pos + 4]);
          int r2 = IntegerHelper.getInt(data[pos + 5], data[pos + 6]);

          int columnMask = IntegerHelper.getInt(data[pos + 7], data[pos + 8]);
          int c1 = columnMask & 0xff;

          // Check that we are not dealing with offsets
          Assert.verify((columnMask & 0xc0000) == 0);

          columnMask = IntegerHelper.getInt(data[pos + 9], data[pos + 10]);
          int c2 = columnMask & 0xff;

          // Check that we are not dealing with offsets
          Assert.verify((columnMask & 0xc0000) == 0);

          ranges.add(new NameRange(sheet1, c1, r1,  c2, r2));
  ...
  NameRecord(Record t, WorkbookSettings ws, int ind, Biff7 dummy) {
    ...
    try {
      ...
      if (data[pos] == cellReference) {
        ...
        while (pos < data.length) {
          int sheet1 = IntegerHelper.getInt(data[pos + 11], data[pos + 12]);
          int r1 = IntegerHelper.getInt(data[pos + 15], data[pos + 16]);
          int r2 = IntegerHelper.getInt(data[pos + 17], data[pos + 18]);
  ...
  public NameRange[] getRanges() {
    return ranges.toArray(new NameRange[ranges.size()]);
  }
